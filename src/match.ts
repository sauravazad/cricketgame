...
export class Match {
  ...
  outBatsman: Player[];
  result: string;
  constructor(teamName: string, overs: number, wickets: number, target: number, players: string[], outcomes: number[], weights: number[]) {
    this.name = teamName;
    this.overs = overs;
    ...
    this.players = players.map(name => {
      return new Player(name, this.outcomes, weights);
    });
  ...
  handleOutBatsman(batsman: Player) {
  ...
  */
  ...
  printSummary() {
    ...
    if (!this.onStrikeBatsman.isOut) {
      console.log(
        `${this.onStrikeBatsman.name}: ${this.onStrikeBatsman.runsScored}${!this.onStrikeBatsman.isOut ? "\u002A" : ""} (${
          this.onStrikeBatsman.ballsFaced
        })`
      );
    }
    if (!this.nonStrikeBatsman.isOut) {
      console.log(
        `${this.nonStrikeBatsman.name}: ${this.nonStrikeBatsman.runsScored}${!this.nonStrikeBatsman.isOut ? "\u002A" : ""} (${
          this.nonStrikeBatsman.ballsFaced
        })\n`
      );
    }
  }
  //  exexute all the balls for an over untill consdition's are true
  playanOver(overNo: number) {
    let ballNo = 1;
    for (ballNo; ballNo <= NUMBER_OF_BALLS_IN_OVERS; ballNo++) {
      if (this.wicketsLeft > 0 && this.matchStatus == matchStatus.IN_PROGRESS) {
        ...
        this.ballId = ballId;
        const currentBall = new Ball(ballId, outcome, isOut);
